...
class RedisBackend(BroadcastBackend):
    ...
    async def connect(self) -> None:
        self._pub_conn = await asyncio_redis.Connection.create(host=self._host,port=self._port,password=self._password)
        self._sub_conn = await asyncio_redis.Connection.create(host=self._host,port=self._port,password=self._password)
